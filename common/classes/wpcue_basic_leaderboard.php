...
{
    class WpCueBasicLeaderboard extends WpCueQuiz_Base
    {
	...
	public function leaderboard_shortcode($atts){
		...
		$type=get_post_meta($leaderboardid,'leaderboardtype',true);
		...
		if($type==1){
			...
			if($quizid){
				...
				$user_query=$wpdb->get_results("select ID,display_name from $wpdb->users where ID in $users",OBJECT_K);
			...
			if($basis==1 || $basis==2){
				if($basis==1){
					$content.='<thead><tr><th>'.__('User','wpcues-basic-quiz').'</th><th>'.__('Points','wpcues-basic-quiz').'</th></tr></thead><tbody>';
					if($order=='top'){
						$leaders=$wpdb->get_results($wpdb->prepare("select a.userid as userid,sum(b.point) as point from $table_name1 a join $table_name2 b where a.instanceid=b.instanceid and a.status=1 and a.mode=1 and a.userid !=0 group by userid order by point desc LIMIT %d",$num),OBJECT_K);
					}
					else{
						$leaders=$wpdb->get_results($wpdb->prepare("select a.userid as userid,sum(b.point) as point from $table_name1 a join $table_name2 b where a.instanceid=b.instanceid and a.status=1 and a.mode=1 and a.userid !=0 group by userid order by point asc LIMIT %d",$num),OBJECT_K);
					}
				}elseif($basis==2){
					$content.='<thead><tr><th>'.__('User','wpcues-basic-quiz').'</th><th>% '.__('Correct Answers','wpcues-basic-quiz').'</th></tr></thead><tbody>';
					if($order=='top'){
				...
				if($order=='top'){
					$leaders=$wpdb->get_results($wpdb->prepare("select b.user_login as username,count(distinct a.quizid) as counter from $table_name1 a join $wpdb->users b where a.userid=b.ID and a.status=1 and a.userid !=0 and a.mode=1 group by a.userid order by counter desc limit %d",$num));
				}else{
