...
{
    class WpCueBasicBadge extends WpCueQuiz_Base
    {
        const POST_TYPE = "wpcuebasicbadge";
		/**
		...
		*/
		protected function init(){
			...
			$this->add_filter('manage_wpcuebasicbadge_posts_columns','new_badge_columns');
		...
		*/
		public static function wpcue_probadge_admin_scripts(){
			wp_register_script( 'wpprocue-badge', WPCUES_BASICQUIZ_URL. '/admin/js/wpprocue-badge.js',array('jquery') );
			wp_enqueue_script('wpprocue-badge');
			$badgeL10n=array('addnew'=>__('Add New','wpcues-basic-quiz'));
			wp_localize_script('wpprocue-badge','badgeL10n',$badgeL10n);
			wp_register_style( 'createquiz', WPCUES_BASICQUIZ_URL.'/admin/css/createquiz.css');
		...
		*/
		...
		public function schedule_cron(){
			...
			if(!(empty($userids))){
				...
				foreach($userids as $userid){
					...
					if(!empty($userbadge[$userid])){$useridbadge=$userbadge[$userid];}else{$useridbadge=array();}
					$this->assignbadge($point,$quizcount,$correctper,$userid,$quizids,$useridbadge);
		...
		public function assignlevel($point,$quizcount,$correctper,$userid,$quizids){
			...
			}else{$nextlevelrank=1;$assignedlevel=0;}
			$args = array(
		...
		public function send_levelmail($newlevelid,$oldlevelid,$userid){
			...
			if(!empty($oldlevelid)){
				$oldlevel=get_post($oldlevelid);$oldtitle=$oldlevel->post_title;
			}else{$oldtitle='';}
			$user=get_user_by('id',$userid);
			$pattern=array('/%%USERNAME%%/','/%%EMAIL%%/','/%%NEWLEVEL%%/','/%%OLDLEVEL%%/');
			$replace=array($user->display_name,$user->user_email,$newlevel->post_title,$oldtitle);
			if(!(empty($wpprocuesetting['basic']['leveladmin']))){
				$emailid=$wpprocusetting['basic']['adminemail'];
