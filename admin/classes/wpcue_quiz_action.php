...
{
    class WpCueQuizAction extends WpCueQuiz_Base
    {
		/**
	...
	/**
			...
			if($post->post_type=='wpcuebasicquiz' && 'trash' != $post->post_status ){
				...
				if ( $post_type_object->public ) {
					if (!(in_array( $post->post_status, array( 'pending', 'draft', 'future' ) ) )) {
						$action= '<a href="' . get_permalink( $post->ID ) . '" title="' . esc_attr( sprintf( __( 'View','wpcues-basic-quiz'), $post->post_title ) ) . '" rel="permalink">' . __( 'View' ) . '</a>';
					$actions['view']=$action;
					}
	 ...
	 } // END class WpCueQuizAction
} // END if(!class_exists('WpCueQuizAction'))
