...
if(!($('#answereditor').is(':empty'))){$confirmation1=confirm('You have already added some answers for earlier selected question type.Are you sure you want to change question type?');
...
if(jQuery.inArray($val, $secondary)!==-1){
	$.ajax({
		...
		success: function(response){
			...
			if(($('#original_post_status').val()==questioneditorL10n.publish)&&($('input[name="savequestion_status"]').val()==1)){$('#questionchanged').val($questionchanged);}
		}
	});
}else{
	$.ajax({
...
$.ajax({
	...
	success: function(response){
		...
		if(($('#original_post_status').val()==questioneditorL10n.publish)&&($('input[name="savequestion_status"]').val()==1)){$('#questionchanged').val(1);}
