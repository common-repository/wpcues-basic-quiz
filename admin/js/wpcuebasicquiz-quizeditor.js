...
if(!($('#quizeditor').is(':hidden'))){alert(quizeditorL10n.editorexist);return false;}
...
$.ajax({
	...
	success: function(response){
		...
		$('#answereditor').tabs();
		var mainarr=[1,2,3,4,5,6,7];
		var qtypearr=[1,2,3,4,7];$val=parseInt($('#questiontype').val(),10);
		if(jQuery.inArray($val,mainarr) != -1){
			$('#answersbox-1').show();
			if(jQuery.inArray($val,mainarr) != -1){
				if($val==3){
					$('#matchquestion').show();
...
if($('#original_post_status').val() != quizeditorL10n.publish){
$par.find(':input').not(':button').each(function(i,elem){
var input=$(elem);
input.prop('disabled',false);
input.data('disableState',0);});
...
$.ajax({
	...
	success: function(data){
		if(data.msg=='success'){
			...
			questtablealternate();
		}else{alert(quizeditorL10n.errormsg);
			$par.find(':input').not(':button').each(function(i,elem){
				var input=$(elem);
...
if($('#original_post_status').val() ==quizeditorL10n.publish){$publishstatus=1;}else{$publishstatus=0;}
...
if(!($('#quizeditor').is(':hidden'))){alert(quizeditorL10n.questeditor);return false;}
	$autodraftsavestatus=$('#original_post_status').val();
	if($autodraftsavestatus === quizeditorL10n.autodraft){alert(quizeditorL10n.draftmsg);return false;}
	$i=0;var $sectionids=[];
...
if((typeof  $questiontype === 'undefined')||($questiontype === 0)){alert(quizeditorL10n.questtype);return false;}
...
if((typeof $answercontent === 'undefined') || ($.trim($answercontent).length===0)){
	if($questiontype==3){
		alert(quizeditorL10n.answerdet+$questnum+quizeditorL10n.leftcolumn);
	}else{
		alert(quizeditorL10n.answerdet+$questnum);}
...
if((typeof $answercontent === 'undefined') || ($.trim($answercontent).length===0)){
	if($questiontype==3){
		alert(quizeditorL10n.answerdet+$questnum+quizeditorL10n.rightcolumn);
	}else{
		alert(quizeditorL10n.answerdet+$questnum);}
		$breakfunc=1;return false;
}
...
}else{$entityorder=parseFloat($('input[name="entityorder[]"]:last').val())+1;}
}
...
$.ajax({
	...
	success: function (data){
		if(data.msg=='saved'){
			...
			reset_quizeditor($questionid);
		}else{alert(quizeditorL10n.savepost);}
	}
});
});
...
$(document).on('change','.questioncorrectanswer',function(){
	if(($('#original_post_status').val()==quizeditorL10n.publish)&&($('input[name="savequestion_status"]').val()==1)){$('#questionchanged').val($questionchanged);}
});
