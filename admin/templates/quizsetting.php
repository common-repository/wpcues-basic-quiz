<div id="quiz_options_section">
			...
			<li>
				<ul class='logindep <?php if(($logindep==0) || (isset($quizmeta['setting']['usl']) && ($quizmeta['setting']['usl'] == 0))){echo 'hiddendiv'; } ?>'>
					...
					<li class='lastunansquest multilogdep <?php if(($multilogdep==0) || ((isset($quizmeta['setting']['ml'])) && ($quizmeta['setting']['ml'] == 0))){echo 'hiddendiv';} ?>'>
						<input type="checkbox" name="firstunanswerd" class='requiredvar' value='1' <?php if(!(empty($quizmeta['setting']['unanshow']))){echo 'checked';} ?> /> 
						<?php _e('Start from page having first unanswered question','wpcues-basic-quiz'); ?> 
	...
	<div id='settab-3'>
		<ul>
				...
				<li class='quizmodedep'><input type='checkbox' name='undiscloseans' class='requiredvar' value='1' <?php if(isset($quizmeta['setting']['undiscloseans']) && ($quizmeta['setting']['undiscloseans']==1)){echo 'checked';} ?>>
				<?php _e('Do not disclose correct answer finally at all','wpcues-basic-quiz'); ?></li>
				<li class='quizmodedep'><input type='checkbox' name='undiscloseanstried' value='1' class='requiredvar' <?php if(isset($quizmeta['setting']['undiscloseanstried']) && ($quizmeta['setting']['undiscloseanstried']==1)){echo 'checked';} ?>>
				<?php _e('Do not disclose correct answer for untried and wrong attempts','wpcues-basic-quiz'); ?> </li>
			</ul>
	</div>	
	<div id='settab-4'>
		<ul class="innertabs"><li><a href='#dialogtab-1'><?php _e('Submit dialog','wpcues-basic-quiz'); ?></a></li>
