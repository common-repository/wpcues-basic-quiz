...
$.ajax({
			...
			alert("<?php _e('could not be trashed','wpcues-basic-quiz'); ?>");
			}
...
jQuery('input[name=badgeimage]').val('');
 ...
 $(document).on('click','#publish',function(e){
	e.preventDefault();
	$badgetitle=$('#title').val();
	if(($badgetitle=== 'undefined')|| ($.trim($badgetitle).length ===0)){
		$('#messagesubmit').html('<p><?php _e('Please enter badge name','wpcues-basic-quiz'); ?></p>');	
		$('#messagesubmit').show();return false;
	}
	$('#quizax').submit();
});
});
...
global $wpdb;
$WpCueBasicBadge=new WpCueBasicBadge(new WpCueQuiz_Config($trial));
$post_type=$WpCueBasicBadge->showPosttype();
...
<div id="titlewrap"><label class="screen-reader-text" id="title-prompt-text" for="title"><?php _e('Enter Badge Name here','wpcues-basic-quiz');?></label>
...
<h3><?php _e('Badge Image','wpcues-basic-quiz'); ?> : </h3>
...
<input id="upload_image_button" type="button" value="<?php _e('Upload Image','wpcues-basic-quiz'); ?>" />
...
<h3><?php _e('Badge Description','wpcues-basic-quiz'); ?> : </h3>
...
<ul>
	<li><input type="checkbox" name="badgemozstatus" value="1" <?php if(!empty($badgemeta['wpcuebadgemozstatus'][0])){echo 'checked';} ?>/> <?php _e('Issue as Mozilla Open Badge','wpcues-basic-quiz'); ?></li>
	<li><ul><li><?php _e('Required Total Points','wpcues-basic-quiz'); ?></li><li><input type='text' name='badgereqdpoints' value='<?php if(isset($badgemeta['wpcuebadgepoint'])){echo $badgemeta['wpcuebadgepoint'][0];}else{echo 0;}?>'></li></ul></li>
	<li><ul><li><?php _e('Required average % correct answers from all tests','wpcues-basic-quiz'); ?></li><li><input type='text' name='badgecorrans' value='<?php if(isset($badgemeta['wpcuebadgecorper'])){echo $badgemeta['wpcuebadgecorper'][0];}else{echo 0;}?>'></li></ul></li>
	<li><ul><li><?php _e('Required number of unque tests taken','wpcues-basic-quiz'); ?></li><li><input type='text' name='badgenumquiz' value='<?php if(isset($badgemeta['wpcuebadgequiznum'])){echo $badgemeta['wpcuebadgequiznum'][0];}else{echo 0;}?>'></li></ul></li>
	<li><ul><li><?php _e('Must take all exam from Quiz Category','wpcues-basic-quiz'); ?></li><li>
	<?php
