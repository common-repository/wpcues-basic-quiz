...
<div id='savedraft-action'>
<span class="spinner"></span>
<input type="hidden" name="poststatustype" value="1">
<?php 
if($post->post_status != 'publish'){
	 submit_button( __( 'Save Draft' ), 'button', 'publish', false, array( 'id' => 'save') ); 
}?>
</div>
...
<ul>
	...
	<li><?php _e('Number of Questions to load','wpcues-basic-quiz');?> :
		<select name="questloaded" id="questloaded">
			<option value="0" <?php if(empty($basicsetting['questloaded'])){echo 'selected'; }?>><?php _e('All','wpcues-basic-quiz');?></option>
			<?php 
			 if(!empty($quizinfo['totalquestions'])){
						for($i=1;$i<=$quizinfo['totalquestions']-1;$i++){
							echo '<option value="'.$i.'"';
							if(!empty($basicsetting['questloaded'])&& ($basicsetting['questloaded']==$i)){echo 'selected'; }
							echo '>'.$i.'</option>';}
					}
			?>
		</select>
	...
	<li><input type="checkbox" name="addcaptcha" id="addcaptcha" value="1" class="requiredvar" <?php if(!empty($basicsetting['addcaptcha'])){echo 'checked';$captchadep=1;}else{$captchadep=0;}?>><?php _e('Add Captcha','wpcues-basic-quiz');?></li>
	<li class="captchadep <?php if(empty($captchadep)){echo 'hiddendiv';} ?>"><?php _e('Show captcha after question number','wpcues-basic-quiz'); ?>
		<select name="captchalocation" id="captchalocation">
			<?php 
			 if(!empty($quizinfo['totalquestions'])){
						for($i=0;$i<=$quizinfo['totalquestions'];$i++){
							echo '<option value="'.$i.'"';
							if(!empty($basicsetting['captchalocation'])&& ($basicsetting['captchalocation']==$i)){echo 'selected'; }
							echo '>'.$i.'</option>';}
					}
			?>
		</select>
