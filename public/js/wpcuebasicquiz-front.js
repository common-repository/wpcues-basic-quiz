...
if($('#quizmainpage').find('#recaptchadiv').length){
...
$(window).load(function(){default_steps();});
...
if($time==0){clearTimeout(timer);timer=0;$('#quizpost :input').not(':button').prop('disabled',true);
$autosubmission=parseInt($('input[name=autosubmission]').val(),10);
if($autosubmission==1){
$('.submitquizbut').click();
}
...
e.preventDefault();
var matchquestres=[];
if($(this).hasClass('submitquizbut')){
	if($('#recaptchadiv').length){if(!grecaptcha.getResponse(widgetId1)){alert(frontL10n.anscaptcha);return false;}}
	$action='submit';}else{$action='save';}
...
if(($action==frontL10n.submit)&& ($('#quizsubmitdialog').length)){
$('#quizsubmitdialog').dialog({                   
				...
				"OK": function(){
					...
					$.ajax({
						...
						success: function (data){
							if($action==frontL10n.submit){
								$('input[name="instanceid"]').val(0);
								$("#processdiv").hide();
								$('#quizfinalpage').show();
	...
	$.ajax({
		...
		success: function (data){
			if($action==frontL10n.submit){$("#processdiv").hide();
				$('#quizfinalpage').show();
				$('#quizfinalpage').append(data);
				if(typeof timer != 'undefined'){clearTimeout(timer);timer=0;}
				MathJax.Hub.Queue(["Typeset",MathJax.Hub]);
...
if(!($questtoolblock.find('.answercontainer').is(':hidden'))){
...
switch($questiontype){
	...
	case 2:
		...
		break;
	case 3:
		$replies=$questionblock.find('input[name="markmatchquestion"]').prop('checked');
		if($replies){$marked=1;}
		break;
	case 4:
		$checkedanswer=$questionblock.find('input[name="sortquestionstatus"]').prop('checked');
	...
	case 5:
		...
		break;
	case 6:
		$checkedanswer=$questionblock.find('.answerdesc').find('input[name="answer-'+$questionid+'"]').is(':checked');
		if($checkedanswer){$marked=1;}else{$marked=0;}
		break;
	case 7:
		var answer=$questionblock.find('textarea[name="answer-'+$questionid+'"]').val();
		if($.trim(answer)==''){$marked=0;}else{$marked=1;}
		break;
}
if($marked){
	$confirmation=true;
...
if((typeof $errortitle === 'undefined') || ($.trim($errortitle).length===0)){alert(frontL10n.errortitle);return false;}
$errordescinput=$(this).siblings('.reportquestdesc');
$errordesc=$errordescinput.val();
if((typeof $errordesc === 'undefined') || ($.trim($errordesc).length===0)){alert(frontL10n.errordet);return false;}
